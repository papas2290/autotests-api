...
def assert_create_course_response(request: CreateCourseRequestSchema, response: CreateCourseResponseSchema):
    ...
    Сравнивает поля курса из ответа с полями из запроса.
    Для preview_file_id проверяет, что в ответе присутствует вложенная модель файла
    с соответствующим id. Для created_by_user_id проверяет, что в ответе присутствует
    вложенная модель пользователя с соответствующим id.
